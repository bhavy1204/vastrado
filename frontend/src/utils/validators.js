...
export const sellerRegisterSchema = z.object({
    ...
    username: z.string().trim().toLowerCase().min(3, "Username too short").max(30).regex(/^[a-z0-9_]+$/, "Only lowercase letters, numbers, underscores"),
    shopName: z.string().trim().min(2, "Shop name is required").max(80),
    shopDescription: z.string().trim().max(500).optional(),
    shopCategory: z.string().trim().min(1, "Select a category"),
    email: emailSchema,
    password: passwordSchema,
    phone: phoneSchema,
